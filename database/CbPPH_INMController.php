...
class CbPPH_INMController {
    ...
    public function readAll(){
        $query = "SELECT * FROM catastro.plano_ph_inm;";
    ...
    function create($idPlano,$idInmueble){
      $sqlInsert = "INSERT INTO catastro.plano_ph_inm (plano,inmueble)"
    ...
    public function listarPlanos(){
        $query = "SELECT id, codigo FROM catastro.phs;";
        $statement = $this->cdb->prepare($query);
        $statement->execute();
        $rows = $statement->fetchAll(\PDO::FETCH_OBJ);
        return $rows;
   ...
   public function update($id,$inmueble){
    $sqlUpdate = "UPDATE catastro.plano_ph_inm SET inmueble = ".$inmueble." WHERE  id  = ".$id.";";
    try {
   ...
   public function delete($id){
    $sqlDelete =
        "DELETE FROM catastro.plano_ph_inm WHERE id = ".$id.";";
    try {
